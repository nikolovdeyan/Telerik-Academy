...
namespace _16.Groups
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // First create some groups with their respective group numbers and departments:
            List<Group> studentGroups = new List<Group>
            {
                new Group(1, "Mathematics"),
                new Group(2, "Physics"),
                new Group(3, "Medicine"),
            };

            // Now use the Join operator to extract the students from the Mathematics department
            var studentsFiltered = from mathGroup in studentGroups
                                   where mathGroup.DepartmentName == "Mathematics"
                                   join student in studentsList on mathGroup.GroupNumber equals student.GroupNumber
                                   select new
                                   {
                                       name = student.FirstName + " " + student.LastName,
                                       dept = mathGroup.DepartmentName
                                   };

            // Print results
            Console.WriteLine("******************************************");
            Console.WriteLine("Department of Mathematics students:");
            foreach (var student in studentsFiltered)
            {
                Console.WriteLine("Student: {0} -- Department: {1}", student.name, string.Join(", ", student.dept));
            }
            Console.WriteLine();
        }
    }
