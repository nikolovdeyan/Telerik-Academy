...
namespace _03.FirstBeforeLast
{
    class Program
    {
        ...
        {
            // First created a class Student that will be useful for all homework tasks that rely on student information.
            // Create an array of students (using the abovementioned class for the students instances)
            ...
            // Use LINQ to extract all students where the first name is lexicograpically smaller than their last name.
            // Then order the result alphabetically
            var studentList = studentsArr.Where(student => string.Compare(student.FirstName, student.LastName) < 0);

            // Print results
            Console.WriteLine("******************************************");
            Console.WriteLine("Students that have their first name before their last name:");
