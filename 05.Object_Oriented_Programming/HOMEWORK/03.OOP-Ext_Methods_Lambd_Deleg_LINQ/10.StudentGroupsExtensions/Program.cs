...
namespace _10.StudentGroupsExtensions
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Select the students from group number 2 and order them by First Name using Extension Methods
            var studentsFilteredExt = studentsList.Where(x => x.GroupNumber == 2);

            // Print results
            Console.WriteLine("******************************************");
            Console.WriteLine("Students from group 2, sorted in descending order by FirstName:");
            foreach (var student in studentsFilteredExt)
            {
