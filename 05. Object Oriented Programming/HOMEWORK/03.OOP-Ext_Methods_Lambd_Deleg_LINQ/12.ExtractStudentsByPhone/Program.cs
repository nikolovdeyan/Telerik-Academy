...
namespace _12.ExtractStudentsByPhone
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Select the students with phone in Sofia (starting with 2-)
            var studentsFiltered = from student in studentsList
                                   where student.Tel.StartsWith("2-")
                                   select student;

            // Print results
            Console.WriteLine("******************************************");
            Console.WriteLine("Students that have a phone starting with '2-':");
            foreach (var student in studentsFiltered)
            {
                Console.WriteLine("Student: {0} {1} -- phone: {2}", student.FirstName, student.LastName, student.Tel);
            }
            Console.WriteLine();
        }
    }
